...
published. Beta 2.0.67 uses Android build code `410044`. Flutter reuses
`app-release.apk`, so copy and rename the Beta artifact immediately after the
build:

```powershell
New-Item -ItemType Directory -Force .\build\fire-tv\v2.0.67 | Out-Null

# Beta 2.0.67
flutter build apk --release --target-platform android-arm,android-arm64 `
  --build-name 2.0.67 --build-number 410044 --no-tree-shake-icons
Copy-Item .\build\app\outputs\flutter-apk\app-release.apk `
  .\build\fire-tv\v2.0.67\TetoTV-v2.0.67-universal.apk

.\tool\release\verify_release_apk.ps1 `
  -Channel Beta `
  -ApkPath .\build\fire-tv\v2.0.67\TetoTV-v2.0.67-universal.apk

.\tool\release\verify_native_redistribution.ps1 `
  -RequireResolvedBinaries `
  -ResolvedBinaryDirectory .\build\native-playback\outputs `
  -StageBundle -ReleaseTag v2.0.67

.\tool\release\new_release_checksums.ps1 `
  -ApkPath .\build\fire-tv\v2.0.67\TetoTV-v2.0.67-universal.apk `
  -NativeSourcePath .\build\release-compliance\v2.0.67\TetoTV-v2.0.67-native-playback-sources.zip `
  -OutputPath .\build\fire-tv\v2.0.67\SHA256SUMS

.\tool\release\verify_release_payloads.ps1 `
  -Channel Beta `
  -ApkPath .\build\fire-tv\v2.0.67\TetoTV-v2.0.67-universal.apk `
  -NativeSourcePath .\build\release-compliance\v2.0.67\TetoTV-v2.0.67-native-playback-sources.zip `
  -ChecksumsPath .\build\fire-tv\v2.0.67\SHA256SUMS `
  -ReleaseNotesPath .\docs\RELEASE_NOTES_2.0.67.md `
  -ResolvedBinaryDirectory .\build\native-playback\outputs
```
