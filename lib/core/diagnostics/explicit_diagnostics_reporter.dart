...
Map<String, Object?> attachRecentCrashSummaries(
  ...
  for (final summary in ordered) {
    ...
    retained.add({
      ...
      'appVersion': _safeVersion(summary['app_version']?.toString() ?? '0.0.0'),
      'buildNumber': summary['build_number'] is num
          ? (summary['build_number']! as num).toInt().clamp(1, 999999999)
          : 1,
      'buildAttribution':
          summary['app_version'] is String &&
              _safeVersion(summary['app_version']! as String) != '0.0.0' &&
              summary['build_number'] is num &&
              (summary['build_number']! as num) >= 1
          ? 'recorded'
          : 'unknown',
      ...
      if (summary['stack'] case final Object stack)
        'context': {'stack': redactDiagnosticStack(stack.toString())},
    });
  }
...
String buildRedactedDiagnosticsText({
  ...
  final payload = <String, Object?>{
    ...
    // Only the recorder's closed-schema snapshot is accepted here, never an
    // arbitrary widget tree or caller-provided map. Collection is local RAM;
    // this export is only generated by the user's explicit support action.
    'uiRuntime': (uiDiagnostics ?? UiDiagnosticContext.instance).snapshot(),
    'privacy':
        'Explicit user share only. Account and tracker identity, credentials, room codes and access capabilities, display names and avatars, direct media source data, URLs, magnets, torrent hashes, local file paths, email addresses, network addresses, and media bytes are redacted.',
  };
    ...
    return [
      for (final item
        ...
        _sanitizeSnapshot(
          ...
          truncation: truncation,
          aniyomiSafeContext: aniyomiSafeContext,
        ),
    ];
  }
  if (value is Map) {
    ...
    final isAniyomiEvent =
        !aniyomiSafeContext && value['component'] == 'aniyomi-runtime';
    for (final entry in value.entries.take(mapMaximum)) {
      ...
      final enteringAniyomiContext = isAniyomiEvent && rawKey == 'context';
      final safeValue = enteringAniyomiContext
          ? sanitizeDiagnosticContext(entry.value, component: 'aniyomi-runtime')
          : entry.value;
      final childAniyomiSafe = aniyomiSafeContext || enteringAniyomiContext;
      result[key] = !childAniyomiSafe && _isSensitiveDiagnosticKey(rawKey)
          ? '[REDACTED]'
          : _sanitizeSnapshot(
              safeValue,
              depth: depth + 1,
              ...
              aniyomiSafeContext: childAniyomiSafe,
            );
    }
    return result;
  }
