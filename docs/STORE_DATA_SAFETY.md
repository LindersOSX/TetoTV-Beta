...
Reviewed against the application source on September 1, 2026.
...
| User-added manga catalog, extension, and reading activity | In the Developer Mode Manga Preview, adding/browsing an OPDS or declarative repository can send catalog, cover, chapter, page, and acquisition requests. A viewer can also install a Seanime-format manga-provider from a Marketplace repository; it receives manga searches and opaque title/chapter inputs needed for source discovery. A viewer can optionally supply Basic, Bearer, or API-key credentials to an OPDS source. | The user-selected public HTTPS catalog, executable extension, and resource hosts provide manga metadata/pages; requests do not pass through a TetoTV server. OPDS credentials are sent only to the exact source origin; cross-origin reader redirects continue without them. Extensions do not receive TetoTV account/service credentials. | Yes; Developer Mode and the Manga Preview are optional, no catalog, repository, or provider is bundled or recommended, and each source/extension can be removed. | Source/installed-extension definitions, bounded catalog cache, opaque protected library identity, library/progress/preferences, and completed app-private downloads remain locally until removed/reset/uninstall. OPDS credentials and opaque extension title identities use Keystore-backed storage. Page/archive URLs and request headers are not persisted in download jobs, so an interrupted transfer requires source reauthorization/reselection before retry. Remote-host retention is controlled by that host. | **Shared with user-selected third-party hosts.** Executable extension code plus the exact store content, user-generated-content, download, and intellectual-property treatment require release-specific review. |
| Discord Rich Presence activity | When Discord is linked and Rich Presence is enabled, TetoTV can send an anime title, episode, playback state/timing, and artwork. Manga reading can instead send a title, chapter, and page/total; its privacy toggle replaces the manga title with a generic label. Manga artwork is never sent. | Discord displays the user-authorized activity. Manga activity contains no source, repository, catalog, cover, or page URL, request credential/header, private-server identity, or download path. | Yes; Discord connection and Rich Presence can be omitted or disabled. Manga activity additionally requires Developer Mode and an open reader. | TetoTV keeps no server-side Rich Presence history. Local Discord tokens remain in Keystore-backed storage until unlink/reset/uninstall; Discord controls its own processing and retention. | **Shared with Discord at the user's direction.** Confirm the current store definition for app activity and user-initiated sharing. |
| Search, catalog, list, and tracking activity | Public title IDs, search terms and filters, list status, episode progress, bounded catalog lookups, and linked-account profile verification are sent when the corresponding feature is used. | AniList, MyAnimeList, SIMKL, Kitsu, Jikan, AniZip, AniSkip, Cinemeta, and artwork hosts provide the requested catalog, mapping, skip-time, image, profile-verification, or tracking function. SIMKL list/progress sync is not currently enabled. | Account connections and automatic skipping are optional; ordinary catalog/search network use is core to online discovery. | TetoTV keeps bounded local caches as described in `PRIVACY.md`; recipient retention follows each service's policy. | **Shared with the service needed for the user-requested feature.** |
...
| Crash logs and app/device diagnostics | With anonymous crash reporting enabled: app/build, crash category, Android version, ABI, TV/phone class, time, bounded exit/memory metadata, fixed lifecycle markers, and a bounded redacted error/stack. Native reports can include signal/cause plus bounded relative PCs, library basenames, function names, and build IDs; raw tombstones are not sent. A manual support report additionally includes model/manufacturer, playback capabilities, provider identifiers, and a bounded 48-hour technical timeline. | TetoTV HTTPS receiver validates/redacts and posts through the TetoTV bot to restricted Discord crash or diagnostic channels for support and reliability. | Anonymous crash reporting is off by default. Manual diagnostics always require an explicit send action. | Receiver does not retain the body after Discord accepts it. Discord retains the posted report according to channel/platform retention until a moderator deletes it. Local rings retain at most 48 hours and twelve crash/ANR summaries. | **Collected by TetoTV support infrastructure and processed by Wispbyte and Discord.** No advertising or cross-service profiling. |
| Private privacy requests | Request type, requester-supplied reply contact, and bounded request details submitted through the account-free private form. The form warns against including passwords, tokens, payment information, or a date of birth. | The TetoTV companion applies strict origin/schema/size/rate limits and sends accepted requests only to a configured private TetoTV staff channel through Discord. | Yes; submitting the form is user initiated. | The companion does not retain the body after Discord accepts it. The staff message remains in the private Discord channel until staff resolves and deletes it. Wispbyte and Discord process ordinary connection metadata under their own policies. | **Collected by TetoTV support infrastructure and processed by Wispbyte and Discord.** No public support-server post or Discord account is required. |
...
- Android Keystore-backed credentials after import; local profiles; app
  ...
  provider-health records; and Manga Preview sources, catalog cache, library,
  reading progress, reader preferences, and app-private downloads.
...
4. Provide in-product controls described by the form: disconnect linked
   ...
   sources/downloads/history, reset TetoTV, and submit access, correction,
   deletion, or privacy-question requests through the published private route.
