...
| User-added manga catalog and reading activity | In the Developer Mode Manga Preview, adding/browsing an OPDS or declarative repository can send catalog, cover, chapter, page, and acquisition requests. A viewer can optionally supply Basic, Bearer, or API-key credentials. | The user-selected public HTTPS catalog and resource hosts provide manga metadata/pages; requests do not pass through a TetoTV server. Credentials are sent only to the exact source origin; cross-origin reader redirects continue without them. | Yes; Developer Mode and the Manga Preview are optional, no catalog is bundled or recommended, and each source can be removed. | Source definitions, bounded catalog cache, library/progress/preferences, and completed app-private downloads remain locally until removed/reset/uninstall. Credentials use Keystore-backed storage. Page/archive URLs and request headers are not persisted in download jobs, so an interrupted transfer requires source reauthorization/reselection before retry. Remote-host retention is controlled by that host. | **Shared with user-selected third-party hosts.** The exact store content, user-generated-content, download, and intellectual-property treatment requires release-specific review. |
