...
public class VideoOutput implements TextureRegistry.SurfaceProducer.Callback {
    ...
    VideoOutput(TextureRegistry textureRegistryReference, TextureUpdateCallback textureUpdateCallback) {
        ...
        // TetoTV: only the reported Fire OS model uses the legacy texture path.
        // This does not change MPV's decoder, playback settings, or other devices.
        surfaceProducer = FireTvSurfaceProducer.needsCompatibility(android.os.Build.MANUFACTURER,
                android.os.Build.MODEL, android.os.Build.VERSION.SDK_INT)
                ? new FireTvSurfaceProducer(textureRegistryReference)
